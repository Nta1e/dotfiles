...
class MattermostAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Krunchix patch: threads the bot is already part of (it was @mentioned there or
    ...
    async def _post_message(self, chat_id: str, message: str, reply_to: Optional[str], metadata: _Metadata,
        ...
        if self._reply_mode == "thread":
            ...
            if candidate:
                ...
                self._remember_thread(payload["root_id"])
        return await self._post_preserving_thread(chat_id, payload, metadata)

    _ACTIVE_THREAD_TTL = 3 * 24 * 3600
    _ACTIVE_THREADS_MAX = 500

    def _remember_thread(self, root_id: str) -> None:
        import time
        now = time.monotonic()
        self._active_threads[root_id] = now
        if len(self._active_threads) > self._ACTIVE_THREADS_MAX:
            stale = [k for k, t in self._active_threads.items() if now - t > self._ACTIVE_THREAD_TTL]
            for k in stale:
                del self._active_threads[k]
            while len(self._active_threads) > self._ACTIVE_THREADS_MAX:
                del self._active_threads[next(iter(self._active_threads))]

    def _in_active_thread(self, root_id: str) -> bool:
        import time
        seen = self._active_threads.get(root_id)
        if seen is None:
            return False
        if time.monotonic() - seen > self._ACTIVE_THREAD_TTL:
